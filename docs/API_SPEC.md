...
- 저장된 레시피 연결 해제 (`user_saved_recipes` 삭제)
- 분석한 레시피의 `analyzedBy` NULL 처리
...
분석된 레시피를 컬렉션에 저장한다. analyze 시점에 이미 DB에 저장된 레시피를 사용자 컬렉션에 추가한다.
